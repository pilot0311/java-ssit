...
public class Ex01_sort {
	...
	private static void dispTeamMember(HashMap<TeamVo, ArrayList<MemberVO>> teamMap) {
		...
		while (ir.hasNext()) {
			...
			/*
			 * if (teamList != null) { Iterator<MemberVO> ir2 = teamList.iterator(); int seq
			 * = 1;
			 * 
			 * if( ir2.hasNext() ) ir2.next(); while (ir2.hasNext()) { MemberVO memberVO =
			 * ir2.next(); System.out.printf("  [%d] %s\n",seq++, memberVO.getName()); } //
			 * while } else { System.out.println("  팀원이 존재하지 않습니다."); } // if
			 */
			...
			try {
				...
				while (ir2.hasNext()) {
					...
					// if(memberVO.getName().contains("(팀장)")) continue;
...
/*
 * [3조(6명):박정호] [1] 이상문 [2] 이주영 [3] 정하영 [4] 이동현 [5] 주강민 [2조(7명):박민석] [1] 유희진 [2]
 * 고경림 [3] 임경재 [4] 이지현 [5] 김정주 [6] 김호영 [1조(7명):이경서] [1] 신종혁 [2] 이재영 [3] 송해영 [4]
 * 신기범 [5] 이준희 [6] 김성준
 * 
 */
